/**
 * Workflow Tool Executor API
 * 워크플로우에서 호출하는 개별 도구 실행
 */
...
// 도구 실행 함수 매핑
const toolExecutors: Record<string, (params: any, context: any) => Promise<any>> = {
  // ===== HR/인사 도구 =====
  ...
  get_tasks: async (params, context) => {
    ...
    let query = supabase.from('company_tasks').select('*')
...
export async function POST(request: NextRequest) {
  try {
    const { toolName, params, companyId, agentId, userId } = await request.json()

    if (!toolName) {
      return NextResponse.json(
        { success: false, error: '도구 이름이 필요합니다' },
        { status: 400 }
      )
    }
    ...
    if (!executor) {
      return NextResponse.json(
        { success: false, error: `알 수 없는 도구: ${toolName}` },
        { status: 404 }
      )
    }

    const context = { companyId, agentId, userId }
    ...
    console.error('[Workflow Tool Executor] Error:', error)
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    success: true,
    tools: Object.keys(toolExecutors).map(name => ({
      name,
...
function getToolDescription(toolName: string): string {
  ...
  return descriptions[toolName] || '설명 없음'
}
