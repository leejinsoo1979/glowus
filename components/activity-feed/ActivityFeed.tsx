...
// 자동생성 프로젝트명인지 확인
function isAutoGeneratedProject(projectName: string | null): boolean {
  if (!projectName) return true
  return /^claude-\d{8}-[a-z0-9]+$/i.test(projectName)
}

// 프로젝트명 표시용 포맷 (자동생성이면 생략 또는 간략화)
function formatProjectName(projectName: string | null): string | null {
  if (!projectName) return null
  if (isAutoGeneratedProject(projectName)) return null // 자동생성 프로젝트명은 표시하지 않음
  return projectName
}
...
function ActivityItem({
  ...
  const displayProject = formatProjectName(activity.project)
